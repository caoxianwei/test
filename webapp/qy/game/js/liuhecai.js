				...
				if(!isCheck){
					...
					if(combination=='1'){
						if(num<minBet){
							...
							total = minBet - maxBet;
							mui.alert('至少选择'+total+'个号码',document.title, function() {});
							return;
						}
						var map = {};
	...
	bindBeInput:function(t){
		...
		$("#money").bind("keyup", function(){
			...
			$('#kj'+t+' div.touzhu-active').each(function(i){
				var id=$.trim($(this).attr('data'));
				var bs=$.trim($(this).attr('bs').split('/')[0]);
				optionArray[num]=bettingObj(id,price);
				num++;
				if( val ==10 || val ==11 || val == 12 || val ==13 || val == 14 || val ==15 || val == 16 || val ==21 || val == 22 || val ==23 || val == 24 || val ==25 || val == 26 || val ==27 || val == 28 || val ==29 || val ==30 || val == 31 || val ==32 || val == 33 || val ==34 || val == 35 || val ==36 || val == 37 || val ==38 || val == 39 || val ==40 || val == 41 || val ==42 || val == 43 || val ==44 || val == 45 || val ==46 || val == 47 || val ==48 || val == 49 || val ==50 || val == 51 ) {
					if(totalprice <= 0) {
