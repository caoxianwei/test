var lotteryObj={
	...
	init:function(){
		...
		$.ajax({
			type: "POST",
			url:move.jiangsuk3_currentTime,
			dataType:'json',
			data:{
				mw:mw
			},
				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-jiangsuk3open');},timerObj.intervalAPI);
						}
						$('#openlable').html("开奖中...");
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-jiangsuk3');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-jiangsuk3open');},timerObj.interval);
					}
