				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-gd115open');},timerObj.intervalAPI);
						}
						$('#openlable').html("开奖中...");
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-gd115');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-gd115open');},timerObj.interval);
					}
					...
					if(length>0){
						...
						for (var i = 0; i < length; i++) {
							...
							for ( var j = 0; j < openResult.length; j++) {
								...
								if(j < 4) {
									openhtml=openhtml+'<span class="num bg-red">'+num+'</span><i>+</i>';
								} else if (j == 4){
									openhtml=openhtml+'<span class="num bg-red">'+num+'</span>';
								}else if(j == 5){
									openhtml=openhtml+' <i>=</i><span class="num bg-red">'+num+'</span>';
								}
							}
							var hehtml='';
