...
function loadTable(){
	...
	var debit = $("#isDebitPower").val();
	 $('#dg').datagrid({      
	    url:$.getContextPath() + 'tjSsc/queryHandDrawData.do',
		rownumbers:true,
		singleSelect:true,
		autoRowHeight:false,
		fitColumns:false,
...
function saveResult(sessionId){
	...
	$.messager.confirm('系统提示',"确定要保存开奖结果吗？开奖号码：" + open_result,function(r){
		if(r){
			$.post($.getContextPath() +"tjSsc/openResult.do",{session_id:sessionId,open_result:open_result},
					function(data) {
						if (data == "SUCCESS") {
...
function drawback(sessionId,sessionNO){
	$.messager.confirm('系统提示',"确实要退还本期投注吗？期号：" + sessionNO,function(r){
		if(r){
			$.post($.getContextPath() +"tjSsc/drawback.do",{session_id:sessionId},
					function(data) {
						if (data == "SUCCESS") {
							$.messager.alert("系统提示","保存成功","info",function(){
								window.location.reload();
