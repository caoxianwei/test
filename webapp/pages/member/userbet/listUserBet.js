...
function loadTable(){
	$('#dg').datagrid({      
	    ...
	    columns:[[
	        ...
	        {field:'open_result',title:'开奖结果',width:175},
...
function queryData(){
	...
	reloadTable({
		tableId:'dg',
		params:{user_name:user_name,session_no:session_no,bet_flag:bet_flag,game_name:game_name,play_name:play_name,startDate:startDate,endDate:endDate}
	});
	getValidBetMoney();
}
...
function getValidBetMoney(){
	...
	var startDate = $("#startDate").val();
	var endDate =  $("#endDate").val();
	$.post($.getContextPath() +"userBet/queryValidBetMoney.do",{user_name:user_name,session_no:session_no,game_name:game_name,play_name:play_name,startDate:startDate,endDate:endDate}
	  ,function(data){
		  if(!$.isEmpty(data)){
