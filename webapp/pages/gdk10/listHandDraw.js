...
function loadTable(){
	...
	var debit = $("#isDebitPower").val();
	 $('#dg').datagrid({      
	    url:$.getContextPath() + 'gdk10/queryHandDrawData.do',
		rownumbers:true,
		singleSelect:true,
		autoRowHeight:false,
		fitColumns:false,
	    ...
	    columns:[[
	        ...
	        {field:'open_result',title:'结果',width:330,formatter:function(value,row,index){
	        	var str = "";
	        	if($.isEmpty(value)){
	        		str += '<input id="open_result_'+row.session_id+'" class="dg-input"  style="width:320px;" type="text" /><br/>';
	        	}else{
	        		str += '<input id="open_result_'+row.session_id+'" value='+value+' class="dg-input"  style="width:320px;" type="text" /><br/>';
	        	}
	        	str += '<span style="color:red">每个数字之间用逗号隔开,例：06,02,10,13,03,05,07,08,04,09</span>';
	        	return str;
	        }},
...
function saveResult(sessionId){
	...
	$.messager.confirm('系统提示',"确定要保存开奖结果吗？开奖号码：" + open_result,function(r){
		if(r){
			$.post($.getContextPath() +"gdk10/openResult.do",{session_id:sessionId,open_result:open_result},
					function(data) {
						if (data == "SUCCESS") {
...
function drawback(sessionId,sessionNO){
	$.messager.confirm('系统提示',"确实要退还本期投注吗？期号：" + sessionNO,function(r){
		if(r){
			$.post($.getContextPath() +"gdk10/drawback.do",{sessionId:sessionId},
					function(data) {
						if (data == "SUCCESS") {
							$.messager.alert("系统提示","保存成功","info",function(){
								window.location.reload();
