				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-tjsscopen');},timerObj.intervalAPI);
						}
						$('#openlable').html("开奖中...");
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-tjssc');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-tjsscopen');},timerObj.interval);
					}
				...
				if(code=='200'){
                    ...
                    if(length>0){
                        ...
                        for (var i = 0; i < length; i++) {
                            ...
                            time = new Date(time);
                            openTime =time.getFullYear()+'-'+(time.getMonth()+1)+'-'+time.getDate();
                            ...
                            var all = '<span class="all-list num">总:</span>';
                            longhu = '<span class="all-list num">'+longhu+'</span>';
                            var hehtml='';
                            for ( var j = 0; j < sumItems.length; j++) {
                                var num=sumItems[j];
                                if(j != 0) {
                                    hehtml = hehtml + '<span class="all-list num">' + num + '</span>';
                                };
                            }
                            // html=html+'<li class="open-cell"><div class="title-box"><div class="title">'+title+'</div><div class="time">第'+sessionNo+'期 '+time+'</div></div>';
                            html=html+'<div class="open-number"> <span class="num all-list list-title">'+sessionNo.substring(6,9)+'期:</span>'+openhtml+all+hehtml+longhu+'</div>';
