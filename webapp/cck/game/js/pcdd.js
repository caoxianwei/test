var lotteryObj={
				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-bjpcddopen');},timerObj.intervalAPI);
						}
						$('#openlable').html("开奖中...");
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-bjpcdd');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-bjpcddopen');},timerObj.interval);
					}
	...
	bindKj:function(t){
		if(typeof t=='undefined'){
			$(".touzhu-box").unbind( "click" );
			$(".touzhu-box").bind("click", function(){
			...
			$("#kj"+t+" .touzhu-box").unbind( "click" );
			$("#kj"+t+" .touzhu-box").bind("click", function(){
				if ($(this).hasClass("touzhu-active")){
					$(this).removeClass("touzhu-active");
				}else{
					$(this).addClass("touzhu-active");
					...
					if(length>0){
						...
						for (var i = 0; i < length; i++) {
							...
							for ( var j = 0; j < openResult.length; j++) {
								...
								if(j < openResult.length -3) {
                                    openhtml = openhtml + '<span class="num bg-red">' + num + '</span><span class="symbol">+</span>';
                                }else if(j < openResult.length -2){
                                    openhtml = openhtml + '<span class="num bg-red">' + num + '</span>';
								} else if( j < openResult.length -1) {
                                    var color = '';
                                    console.log(openResult);
                                    var lastNum = openResult[openResult.length - 1];
                                    ...
                                    openhtml = openhtml + '<span class="symbol">=</span><span class="'+color+'">' + num + '</span>';
								}
							}
							var hehtml='';
							...
							// html=html+'<div class="data-box"><div class="title"><div class="he">总和</div><div class="longhu">波色</div></div>';
