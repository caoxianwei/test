...
$('.download').attr('href','http://'+serve+'/download/');
setTimeout(function () {
    $('#app_download').attr('href','http://'+serve+'/download/');
});
// 闪烁
...
function setCookie(json) {
    ...
    $.cookie('logo',json.logo,{path:"/"});
    $.cookie('type',json.userGrade,{path:"/"});
    $.cookie('userName',json.userName,{path:"/"});
}
$(function () {
//关闭广告栏目
    $("#_rightAD .close_btn,#_leftAD .close_btn").click(function () {
        $(this).parent().hide();
    })
    ...
    $.ajax({
        ...
        success: function (json) {
            if(json.code != 200){
                ...
                $("#online-serve").attr('href',data);
                $("#kefu").attr('href',data);
                $.cookie('kefu',data,{path: '/'});
            }
        }
    });
...
var scriptObj = document.querySelectorAll('script');
var linkObj = document.querySelectorAll('link');
for(var i = 0; i < scriptObj.length; i ++) {
    if(scriptObj[i].src != ''){
        scriptObj[i].src = scriptObj[i].src+'?time='+new Date().getTime();
    }
}
// for(var i = 0; i < linkObj.length; i ++) {
//     if(linkObj[i].href != ''){
//         // console.log(linkObj[i].href);
//         linkObj[i].href = linkObj[i].href+'?time='+new Date().getTime();
//     }
// }