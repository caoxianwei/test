...
if (md.mobile()) {
	if(parent) {
        parent.window.location.href = "http://" + window.location.host + "/game" + "";
    } else {
...
function CountDown(element,divtype){//活动倒计时
 	...
 	function ShowCountDown()
    {
		...
		if (leftTime>0){			
			...
			$(divtype+" .pc span.sp1").text(second.substring(0, 1));
			$(divtype+" .pc span.sp2").text(second.substring(2, 1));
		} else{
...
function fade(banner,cirl,timer,click_left,click_right){//淡入淡出轮播  1.切换的对像	2.小圆点控制的对像	 3.时间值[必选]  4.左点击  5.右点击
	...
	function bann_auto(){
			i++;
			if(i>olen-1) i=0;
			$(cirl).eq(i).addClass('active').siblings().removeClass('active');
			$(banner).eq(i).css("zIndex","1").animate({"opacity":"1"},600,'swing').siblings().css("zIndex","0").animate({"opacity":"0"},600,'swing');
		}
	$(cirl).hover(function(){
